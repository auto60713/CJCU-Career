		...
		<div class="scrollable">
			...
			<?php foreach ($users as $user) : ?>
				...
				<div class="list-item with-icon" data-id="<?php echo $user->id ?>" data-role="<?php echo $user->role_name ?>">
					<?php //echo gravatar_link($user->email, 32,'',$user->last_name.'&nbsp;'.$user->first_name); ?>
					<img src="<?php echo Template::theme_url('images/user.png') ?>">
					<p>
						<?php 
							if ($this->settings_lib->item('auth.use_own_names'))
							{ 
								$name = abbrev_name($user->last_name.' '.$user->first_name);
			...
			<div class="padded">
				...
				<div class="row" style="margin-bottom: 2.5em">
					<div class="column size1of2">
						...
						<span class="big-text"><b><?php echo $user_count ?></b></span> &nbsp; 使用者
					</div>
					<div class="column size1of2 last-column">
						<img src="<?php echo Template::theme_url('images/user.png') ?>" style="vertical-align: bottom; position: relative; top: -5px; margin-right: 1em;" />	
						
						<span class="big-text"><b><?php echo $deleted_users ?></b></span> &nbsp; <?php echo anchor(SITE_AREA .'/settings/users/deleted', '已停用的使用者', 'class="ajaxify"') ?>
					</div>
				</div>
